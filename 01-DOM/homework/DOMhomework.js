...
// Tu codigo acá:
let toDoItems = [];//{description: "Limpiar", complete: false/true},{},{}
...
let span = document.querySelector("#createdBy");//selecciona el elemento span
span.innerHTML +=  " Carlos Mario"; //acceder al texto para modificarlo
//también pude ser así:
//let span = document.querySelector("#createdBy").innerHTML +=  " Carlos Mario";
...
function ToDo (description) {
  // Tu código acá:
 this.description = description;//como estamos en un constructor, las propiedades van con un this
 this.complete = false;
};
//let nuevoElemento = new ToDo("algo"); De esta forma creamos y/o instanciamos una nueva intancia de ToDo
...
// Tu código acá:
ToDo.prototype.completeToDo = function () { //creación del prototyppe, la función va sin nombre y sin parámetros
  // Tu código acá:
 this.complete = true;
};
//let nuevaTarea = New Todo('Limpiar la casa');  {description = description; complete = false};
//nuevatarea.completeTodo(); {description = description; complete = true};
...
function buildToDo(todo, index) {
  ...
  //toDoShell.setAttribute = ("class", "toDoShell");// se puede asignar porque ya se creo un elemento del DOM

  //3) Crear un elemento 'span' y asignárselo a una variable denominada 'toDoText'
  let toDoText = document.createElement("span");//<span>Limpiar la casa</span>
 ...
 //4)Utilizando el objeto toDo pasado como argumento, setear el 'toDoText' innerHTML asignándole el valor de la propiedad 'description' del objeto ToDo.
  ...
  //5) Asignarle como id del 'toDoText' el valor 'index' recibido como argumento
  toDoText.id = index; //<span id = "toDoText">Limpiar la casa</span>
  //toDoText.setAttribute("id", index); De esta forma también podemos asignar el punto 5
  
  if(todo.complete === true){
    toDoText.className = "completeText"; //<span id = "toDoText" class = ""completeText> </span>
    //toDoText.setAttribute = ("class", completeText");
  }
  //7) Agregar 'toDoText' como hijo de 'toDoShell'
  toDoShell.appendChild(toDoText);//el appendChild pide que se le ingrese un nodo como padre
  //<div class = "toDoShell"><span id = "toDoText">Limpiar la casa</span></div>
...
function buildToDos(toDos) {
  ...
  let newArray = toDos.map((todo, index) => buildToDo(todo, index));//la función buildToDo recibe los parámetros(todo, index), por eso se ponen 
  //[{description: "Limpiar", complete: false/true},{},{}]
  return newArray;
...
function displayToDos() {
  ...
  toDoContainer.innerHTML = '';//Se pone como vacío par que no repita lo enviado anteriormente
  
  let result = buildToDos(toDoItems);//guardar lo que arroje la funcion 
...
//  1) Crear un nuevo ToDo usando la clase ToDo y pasándole el valor del input 'toDoInput' como parámetro newToDo 
...
function addToDo() {
  // Tu código acá:
  let input = document.querySelector("#toDoInput");//Toma el valor del input
  let newToDo = new ToDo(input.value);//crea o instanciar un nuevo elemento

  toDoItems.push(newToDo);//pushea el nuevo elemento. Aquí es agregar el elemento más no agregar un hijo, por eso no se usa appendChild
  toDoInput.value = '';//lo deja en vacío, porque sino, se va a pushear elementos repetidos, limpia lo que se vé en pantalla
  displayToDos();
};
...
//Forma larga
let boton = document.querySelector("#addButton");//seleccionamos el elemento
boton.addEventListener("click", addToDo);//recibirá un evento click y ejecutará la función addToDo
...
function completeToDo(event) {
  ...
  // Tu código acá:
  toDoItems[index].completeToDo();//el index sale del indice de las tareas agregadas por el usuario, en este caso en el primero
  //{description: "Limpiar", complete: false/true},{},{}
  displayToDos();
};
